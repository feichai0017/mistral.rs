...
pub use paged_attention::OxidePagedDecodeStats;
...
impl MistralRs {
    ...
    pub async fn oxide_paged_decode_stats(
        &self,
        model_id: Option<&str>,
    ) -> Result<Option<OxidePagedDecodeStats>, MistralRsError> {
        let resolved_model_id = self.resolve_alias_or_default(model_id)?;
        let pipeline = {
            let engines = self
                .engines
        ...
        let stats = pipeline.lock().await.oxide_paged_decode_stats();
        stats.map_err(|err| {
            MistralRsError::Other(format!(
                "failed to read Oxide paged-decode stats for model `{resolved_model_id}`: {err}"
            ))
        })
    }
